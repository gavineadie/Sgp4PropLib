...
// as listed in the DLL Prerequisites section of the accompanying documentation, before using this DLL.
// 
// When the function is called, the GEO model is set to WGS-72 and the FK model is set to FK5.
// If the user plans to use the SGP4 propagator, do NOT change this default setting. Otherwise, SGP4 won't work
//
...
typedef int (STDCALL *fnPtrEnvGetFkIdx)(void);
...
typedef int (STDCALL *fnPtrEnvGetGeoIdx)(void);
...
typedef __int64 (STDCALL *fnPtrEnvGetFkPtr)(void);
...
typedef int (STDCALL *fnPtrEnvGetEarthShape)(void);
...
void LoadEnvConstDll(void);
void FreeEnvConstDll(void);
